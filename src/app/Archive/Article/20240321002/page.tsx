"use client";

import { useState } from "react";
import Image from "next/image";
import Link from "next/link";
import { MdDownloadForOffline } from "react-icons/md";

export default function ArticleTabs() {
    const [activeTab, setActiveTab] = useState("info");
    const orchidIcon = "/images/orcid.logo.icon.svg";

    const tabs = [
        { id: "info", label: "Information" },
        { id: "abstract", label: "Abstract" },
        { id: "fulltext", label: "Full Text" },
        { id: "references", label: "References" },
        { id: "appendix", label: "Appendix" },
    ];

    return (
        <div className="w-full h-20 flex">
            <div className="home-body-text w-[73%] px-3 ">
                <div className="  mx-auto px-4 py-1">
                    <div className="journal-Container"></div>
                    <h1 className="text-[29px] font-bold leading-[32px] text-[#0B5486] article-info-title">
                        Assessment of sonographic measurement of the median nerve cross-sectional area in pregnant women with hypertensive disorders
                    </h1>
                </div>
                        <div className="mb-6 text-sm flex flex-wrap gap-2">
                            {/* Deniz Oluklu */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0002-9050-2041" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Deniz Oluklu,</span>
                            </div>

                            {/* Sule Goncu Ayhan */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0002-5770-7555" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Sule Goncu Ayhan,</span>
                            </div>

                            {/* Dilek Menekse Beser */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0002-7022-0610" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Dilek Menekse Beser,</span>
                            </div>

                            {/* Derya Uyan Hendem */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0003-1866-7295" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Derya Uyan Hendem,</span>
                            </div>

                            {/* Ezgi Turgut */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0002-5509-7888" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Ezgi Turgut,</span>
                            </div>

                            {/* Egemen Ayhan */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0002-0324-3126" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Egemen Ayhan,</span>
                            </div>

                            {/* Dilek Sahin */}
                            <div className="flex items-center space-x-2">
                                <Link href="https://orcid.org/0000-0001-8567-9048" target="_blank">
                                    <Image
                                        src={orchidIcon}
                                        alt="ORCID"
                                        width={16}
                                        height={16}
                                        className="inline ml-1"
                                    />
                                </Link>
                                <span className="text-blue-600 relative top-[1.5px]">Dilek Sahin</span>
                            </div>
                        </div>


                {/* Tabs */}
                <ul className="flex border-b border-gray-300 mb-4">
                    {tabs.map((tab) => (
                        <li key={tab.id} className=" list-none">
                            <button
                                onClick={() => setActiveTab(tab.id)}
                                className={`py-2 px-4 font-normal text-[14px] leading-[20px] rounded-t-md
                                ${activeTab === tab.id
                                        ? "text-[rgb(85,85,85)] border border-b-0 border-gray-300 bg-white mb-[-0.5px]"
                                        : "text-[rgb(0,140,199)] border-none"
                                    }`}
                            >
                                {tab.label}
                            </button>
                        </li>
                    ))}
                </ul>
                {/* Tab Content */}
                <div className="text-gray-800 ml-6 text-sm leading-relaxed">
                    {activeTab === "info" && (
                            <div>
                                <div id="manuscript-evaluation" className="text-[#333333]">
                                    <h2 className="text-[14px] font-bold">Article info</h2>
                                    <p className="text-[14px] font-normal leading-[20px]">
                                        Assessment of sonographic measurement of the median nerve cross-sectional area in pregnant women with hypertensive disorders. Perinatal Journal 2024;32(1):21-25 DOI: 10.59215/prn.24.0321002
                                        <br className="br" />
                                        <br className="br" />
                                    </p>
                                </div>
                                <div id="manuscript-evaluation" className="text-[#333333]">
                                    <h2 className="text-[14px] font-bold">Author(s) Information</h2>
                                    <div className="text-[14px] font-normal leading-[4px]">
                                        <Link href="https://orcid.org/0000-0002-9050-2041" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline pb-[0.5px] ml-1" />
                                        </Link>
                                        &nbsp;Deniz Oluklu<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Study Supervisor, Data Collector, Data Analyst, Results Interpreter, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0002-5770-7555" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Sule Goncu Ayhan<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Study Supervisor, Results Interpreter, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0002-7022-0610" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Dilek Menekse Beser<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Study Supervisor, Data Collector, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0003-1866-7295" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Derya Uyan Hendem<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Data Collector, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0002-5509-7888" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Ezgi Turgut<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Results Interpreter, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0002-0324-3126" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Egemen Ayhan<sup>2</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Study Supervisor, Results Interpreter, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                        <Link href="https://orcid.org/0000-0001-8567-9048" target="_blank">
                                            <Image src={orchidIcon} alt="ORCID" width={16} height={16} className="inline ml-1" />
                                        </Link>
                                        &nbsp;Dilek Sahin<sup>1</sup>
                                        <span className="text-[11px] leading-[20px] font-normal not-italic text-[#2080C2]">
                                            &nbsp;(Project Initiator, Study Designer, Study Supervisor, Results Interpreter, Literature Researcher, Manuscript Writer, Final Reviewer)
                                        </span>
                                        <div className="br" />
                                    </div>
                                </div>
                                <div className="br" />
                                <div className="br" />
                                <div id="manuscript-evaluation" className="text-[#333333]">
                                    <div className="text-[14px] font-normal leading-[20px]">
                                        1. Turkish Ministry of Health Ankara City Hospital, Department of Obstetrics and Gynecology, Division of Perinatology, Ankara, Türkiye
                                        <br />
                                        2. Diskapi Yildirim Beyazit Training and Research Hospital, University of Health Sciences Turkey, Department of Orthopedics and Traumatology, Ankara, Türkiye
                                        <div className="br" />
                                    </div>
                                </div>
                                <h3 className="mt-4 font-semibold">Correspondence</h3>
                                <p>
                                    Deniz Oluklu, Turkish Ministry of Health Ankara City Hospital, Department of Obstetrics and Gynecology, Division of Perinatology, Ankara, Türkiye,
                                    <br className="br" />
                                    <a href="mailto:denizoluklu9@gmail.com" className="text-blue-600 underline">
                                        denizoluklu9@gmail.com
                                    </a>
                                </p>

                                <h3 className="mt-4 font-semibold">Publication History</h3>
                                <ul className="list-disc pl-5">
                                    <li>Manuscript Received: December 08, 2023</li>
                                    <li>Manuscript Accepted: February 02, 2024</li>
                                    <li>Earlyview Date: February 02, 2024</li>
                                    <li>Publication date: April 01, 2024</li>
                                </ul>

                                <h3 className="mt-4 font-semibold">Conflicts of Interest</h3>
                                <p>No conflicts declared.</p>
                            </div>

                    )}

                    {activeTab === "abstract" && (
                        <div>
                            <h2 className="font-semibold text-lg mb-2">Abstract</h2>

                            <h3 className="mt-4 font-semibold">Objective</h3>  
                            <p>
                                To investigate the prevalence of carpal tunnel syndrome symptoms and assessed the median nerve cross-sectional area measurement by ultrasound in pregnant women in the second and third trimester with hypertensive disorders.
                            </p>

                            <h3 className="mt-4 font-semibold">Methods</h3>
                            <p>
                                This study presented four groups that were included in the cross-sectional study: one group of healthy pregnant women (control group) and three groups of pregnant women with hypertensive disorders (gestational hypertension, chronic hypertension, and preeclampsia).
                            </p>

                            <h3 className="mt-4 font-semibold">Results</h3>
                            <p>
                                Carpal tunnel syndrome symptoms such as hand numbness and hand pain were significantly higher in the hypertensive group than in the control group (p=.013, p=.002, respectively). We found that the right and left median nerves’ cross-sectional area was significantly higher in the hypertensive group than in the control group (p&lt;.001). Also, participants had a positive low correlation and statistically significant difference between body mass index, gestational weight gain, and median nerve cross-sectional area (r= .347, p&lt;.001; r=.414, p&lt;.001). We showed excellent reliability for intraclass correlation coefficients of right and left median nerve cross-sectional area measurements (Intra observer Intraclass correlation coefficients= 0.926 and 0.955, respectively).
                            </p>

                            <h3 className="mt-4 font-semibold">Conclusion</h3>
                            <p>
                                Obstetricians should be aware of the increased prevalence of carpal tunnel syndrome in pregnant women with hypertensive disorders and increased carpal tunnel syndrome symptoms in the second and third trimesters. Because of its convenience and low cost, ultrasonography may be preferred for diagnosing carpal tunnel syndrome symptoms in pregnant women. In addition, pregnant women should be referred for early diagnosis and treatment of carpal tunnel syndrome.
                            </p>

                            <h3 className="mt-4 font-semibold">Keywords</h3>
                            <p>Carpal tunnel syndrome, hypertensive disorders, median nerve, pregnancy</p>
                        </div>


                    )}

                    {activeTab === "fulltext" && (
                        <div>
                            <h2 className="font-semibold text-lg mb-2">Full Text</h2>
                            <p></p>
                        </div>
                    )}

                    {activeTab === "references" && (
                        <div>
                            <h2 className="font-semibold text-lg mb-2">References</h2>
                            <p></p>
                            
                        </div>
                    )}

                    {activeTab === "appendix" && (
                        <div>
                            <h2 className="font-semibold text-lg mb-2">Appendix</h2>

                            {/*<table className="table table-bordered table-striped">
                                    <thead>
                                        <tr>
                                        <th></th>
                                        <th>File/Description</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                
                                        <tr>
                                        <td className="pull-center width">
                                            <svg className="pl-4 pb-4" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 30 30">
                                            <path d="M24.707,8.793l-6.5-6.5C18.019,2.105,17.765,2,17.5,2H7C5.895,2,5,2.895,5,4v22c0,1.105,0.895,2,2,2h16c1.105,0,2-0.895,2-2V9.5C25,9.235,24.895,8.981,24.707,8.793z M18,10c-0.552,0-1-0.448-1-1V3.904L23.096,10H18z"></path>
                                            </svg>
                                        </td>
                                        <td className="pl-3">
                                            <strong>
                                            <Link download href="/content/appendix/earlyview//PF-2025-04-28-112956.docx" target="_blank">
                                                Table 1 <i className="icon-share"></i>
                                            </Link>
                                            </strong>
                                            <br />
                                            Demographic parameters, measurements and pregnancy outcomes of the patients
                                        </td>
                                        </tr>

                                    </tbody>
                                </table> */}
                        </div>
                    )}
                </div>
            </div>

            <div className="home-body-right-sidebar w-[25%]">
                <div className="border border-gray-300 rounded-md bg-white shadow-sm text-sm text-gray-800 w-full max-w-xs flex flex-col overflow-hidden">
                    {/* Sections with dividers */}
                    <div className="divide-y divide-gray-300">
                        
                        {/* Article's Type */}
                        <div className="p-3">
                            <p className="font-semibold">Article's type</p>
                            <p>Original Article</p>
                        </div>

                        {/* Pages */}
                        <div className="p-3">
                            <p className="font-semibold">Pages</p>
                            <p>21-25</p>
                        </div>

                        {/* Article Info */}
                        <div className="p-3 space-y-1">
                            <p className="font-semibold">Article info</p>
                            <p>
                                <span className="font-semibold">Online publication date:</span>{" "}
                                February 02, 2024
                            </p>
                        </div>
                        <div className="p-3">
                            <p>Perinatal Journal 2025; 32 (1)</p>
                        </div>

                        {/* DOI */}
                        <div className="p-3">
                            <p className="font-semibold">DOI</p>
                            <a
                                href="https://dx.doi.org/10.59215/prn.24.0321002"
                                className="text-sky-600 hover:underline"
                            >
                                10.59215/prn.24.0321002
                            </a>
                        </div>
                    </div>

                    {/* Download as PDF Button */}
                    <div className="p-3 border-t border-gray-300">
                        <a
                            download={true}
                            href="/content/pdf/v33i1/Article3.pdf"
                            type="button"
                            className="w-full !bg-[#5EBD5E] hover:!bg-green-600 !text-white font-medium py-2 px-4 rounded flex items-center justify-center space-x-2"
                        >
                            <MdDownloadForOffline className="text-white text-lg" />
                            <span>Download as PDF</span>
                        </a>
                    </div>
                </div>

            </div>
        </div>
    );
}
